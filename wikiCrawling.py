...
def main() :
    ...
    countryLinks = getLink("https://ko.wikipedia.org/wiki/%EB%B6%84%EB%A5%98:%EB%82%98%EB%9D%BC%EB%B3%84_%EB%B0%B0%EA%B2%BD%EC%9C%BC%EB%A1%9C_%ED%95%9C_%EC%98%81%ED%99%94")
    ...
    if countryLinks :
        for country in countryLinks  :
            cityLinks = getLink("https://ko.wikipedia.org"+countryLinks[country])
            if cityLinks :
                for city in cityLinks : 
                    cityMovies[city] = getTitle("https://ko.wikipedia.org"+cityLinks[city])
                countryMovies[country]= cityMovies
            else : 
                countryMovies[country] = getTitle("https://ko.wikipedia.org"+countryLinks[country])
